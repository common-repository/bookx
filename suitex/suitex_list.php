...
class suitex_list {
    ...
    */
 	...
 	function startList($headers, $link, $order, $sort, $rows, $limit, $number, $hidden=''){
		...
		if ($this->filters || $this->orderForm || $this->dateFilter){
        ...
        if ($this->paging){
            $pager = "<div class=\"tablenav-pages\">" . $this->createPaging($limit, $number, $_SERVER["PHP_SELF"] . "?" . $_SERVER["QUERY_STRING"]) . "</div><br class=\"clear\" />"; 
            $text .= $pager;
        } 
    ...
    */
    ...
    function createPaging($limit, $number, $url){
